...
## 코드 리뷰로 다중행 쓰기의 원자성 결함을 발견하고 RPC 패턴으로 일원화

**Problem**
Phase 2(매칭) 진입 전 자체 코드 리뷰에서, 코스 장소 순서 변경(`reorderPlaces`) 과 가용 시간 저장(`setAvailableTimes`) 이 클라이언트에서 여러 행을 **순차 UPDATE / DELETE+INSERT** 로 처리하고 있었다. 중간에 네트워크가 끊기면 일부 행만 갱신된 상태로 남아 사용자가 보는 데이터와 DB 상태가 불일치하는 결함. 또한 RLS 만으로는 "전달된 place 들이 정말로 본인 코스의 것인가" 같은 관계 검증이 어려워, 본인이 소유하지 않은 코스의 장소 순서를 조작할 수 있는 잠재 권한 우회 경로가 존재.

**Action**
두 경로 모두 `security definer` RPC 로 교체:
- `reorder_places(p_course_id uuid, p_order_pairs jsonb)` — `auth.uid()` 가드 → 코스 소유자 검증 → "전달된 place 들이 모두 그 코스 소속인지" 카운트 매칭 검증 → `update places ... from (jsonb_array_elements ...) ` 단일 트랜잭션 UPDATE
- `set_available_times(p_slots jsonb)` — `auth.uid()` 가드 → 슬롯 enum/범위 검증 → `delete ... where user_id = v_uid` + `insert ... select v_uid, ... from jsonb_array_elements` 단일 트랜잭션

이전에 이미 적용된 마이그레이션을 편집하지 않기 위해 새 마이그레이션 파일(`20260524000003`, `20260524000004`) 로 분리. 더불어 동일 리뷰에서 발견된 `update_ideal_type` 의 시그니처 보정(`default null`) 도 같은 immutability 원칙대로 별도 파일(`20260524000005`) 로 처리하고, 결정 문서(`docs/decisions/migration-immutability.md`) 로 원칙 명문화.

**Result**
다중행 쓰기 경로 2개의 원자성 확보, 권한 검증이 RPC 안으로 들어와 서버 단일 책임 모델 유지. 클라이언트 코드는 `await supabase.rpc(...)` 한 줄로 단순화되어 호출 지점에서 부분 실패 처리 로직 자체가 제거됨. 마이그레이션 immutability 가 first-class 원칙으로 문서화되어 향후 보정 작업의 판단 비용 제거.
